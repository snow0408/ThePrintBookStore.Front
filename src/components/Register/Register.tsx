...
function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      navigate("/CheckVerificationCode", {
        state: { email: formData.email },
      });
      alert("註冊成功！");
    } catch (error: unknown) {
      if (axios.isAxiosError(error)) {
        console.error("該信箱已註冊", error.response?.data);
        alert("該信箱已註冊" + error.response?.data);
      } else {
        console.error("該信箱已註冊", error);
        alert("該信箱已註冊" + String(error));
      }
    }
  };
            ...
            <div className="login-area">
              <form onSubmit={handleSubmit}>
                ...
                <div className="mb-5">
                  ...
                  <label htmlFor="terms">
                    我已閱讀並同意
                    <a href="/terms-and-conditions">註冊須知</a>
                  </label>
                </div>
                {/* <div className='btn btn-primary   '> */}
                <button
                  type="submit"
                  className="btn btn-primary w-100 me-2 btnhover"
                >
                  註冊
                </button>
                {/* </div> */}
