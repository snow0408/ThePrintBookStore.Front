...
const Datatables: FC<DatatablesProps> = () => {
  ...
  const filteredData = data.filter(
    (item: { [key: string]: string | number | boolean }) => {
      ...
      const matchesUsingStatus =
        !usingStatus || item.usingStatus === usingStatus;
  ...
  const handleSaveCoupon = async () => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/Coupon`, newCoupon);
